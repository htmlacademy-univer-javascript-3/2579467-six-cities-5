...
export const OfferPage = () => {
  ...
  useEffect(() => {
    if (!isOfferLoading && !currentOfferData) {
      navigate(AppRoute.NotFoundPage);
    }
  }, [isOfferLoading, currentOfferData, navigate]);

  const handleItemHover = (offerId: string) => {
    const currentPoint = offersNearby.find((offer) => offer.id === offerId) || null;
    setSelectedPoint(currentPoint);
  };

  if (currentOfferData && offersNearby.length > 0) {
    const currentOffer = offers.find((offer) => currentOfferData.id === offer.id);
        ...
        <main className="page__main page__main--offer">
          <section className="offer">
            ...
            <section className="offer__map map">
              {currentOffer && <Map city={offersNearby[0].city} points={[...offersNearby.slice(0, 3), currentOffer]} selectedPoint={selectedPoint}/>}
            </section>
          </section>
